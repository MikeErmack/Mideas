import json
import requests
from datetime import datetime, timedelta
import time

# ===================== CONFIG =====================
API_KEY = "d59d428ff9e5500c8b99749170920012"
TELEGRAM_BOT_TOKEN = "8781452746:AAHHgoLYPN-KK4t4xCO_JqYqIRxC8NV3HMU"
TELEGRAM_CHAT_ID = "793027389"

headers = {'x-apisports-key': API_KEY}

# Leagues (accurate IDs)
LEAGUES = {
    39: "Premier League",
    140: "La Liga",
    78: "Bundesliga",
    135: "Serie A",
    61: "Ligue 1",
    88: "Eredivisie",
    94: "Primeira Liga",
    307: "Saudi Pro League",
    218: "Austrian Bundesliga",
    144: "Belgian Pro League",
    1: "World Cup"                    # World Cup 2026
}

# ===================== API CALL TRACKER =====================
class APICounter:
    def __init__(self):
        self.calls = 0
        self.max_calls = 85  # Safety margin under 95

    def request(self):
        self.calls += 1
        if self.calls > self.max_calls:
            print(f"⚠️  API call limit reached ({self.calls}). Stopping to stay safe.")
            raise Exception("API limit reached")

api = APICounter()


def send_telegram_message(message):
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
        payload = {"chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "HTML"}
        requests.post(url, json=payload, timeout=20)
        print("✅ Telegram sent")
    except Exception as e:
        print(f"❌ Telegram error: {e}")


def get_fixtures_next_3_days():
    """Fetch fixtures for next 3 days - ~11 calls max"""
    today = datetime.now()
    from_date = today.strftime("%Y-%m-%d")
    to_date = (today + timedelta(days=3)).strftime("%Y-%m-%d")

    fixtures = []

    for league_id, league_name in LEAGUES.items():
        try:
            api.request()
            url = "https://api-football-v1.p.rapidapi.com/v3/fixtures"
            params = {
                "league": league_id,
                "season": 2026,
                "from": from_date,
                "to": to_date,
                "status": "NS"
            }
            resp = requests.get(url, headers=headers, params=params, timeout=15)
            if resp.status_code == 200:
                for f in resp.json().get('response', []):
                    fixtures.append({
                        "fixture_id": f['fixture']['id'],
                        "date": f['fixture']['date'][:10],
                        "league": league_name,
                        "league_id": league_id,
                        "home_team": f['teams']['home']['name'],
                        "away_team": f['teams']['away']['name'],
                        "home_id": f['teams']['home']['id'],
                        "away_id": f['teams']['away']['id']
                    })
            time.sleep(0.6)
        except:
            continue

    return fixtures


def get_standings(league_id):
    """Get current standings - 1 call per league"""
    try:
        api.request()
        url = "https://api-football-v1.p.rapidapi.com/v3/standings"
        params = {"league": league_id, "season": 2026}
        resp = requests.get(url, headers=headers, params=params, timeout=10)
        if resp.status_code == 200:
            data = resp.json().get('response', [{}])[0]
            return data.get('league', {}).get('standings', [[]])[0]
    except:
        pass
    return []


def get_team_stats(team_id, league_id):
    """Get team statistics - 1 call"""
    try:
        api.request()
        url = "https://api-football-v1.p.rapidapi.com/v3/teams/statistics"
        params = {"team": team_id, "league": league_id, "season": 2026}
        resp = requests.get(url, headers=headers, params=params, timeout=10)
        if resp.status_code == 200:
            return resp.json().get('response', {})
    except:
        pass
    return {}


def get_head_to_head(home_id, away_id):
    """Get last 6 H2H - 1 call"""
    try:
        api.request()
        url = "https://api-football-v1.p.rapidapi.com/v3/fixtures/headtohead"
        params = {"h2h": f"{home_id}-{away_id}", "last": 6}
        resp = requests.get(url, headers=headers, params=params, timeout=10)
        if resp.status_code == 200:
            return resp.json().get('response', [])
    except:
        pass
    return []


class PreMatchAnalyzer:
    def __init__(self):
        self.standings_cache = {}

    def get_league_standings(self, league_id):
        if league_id not in self.standings_cache:
            self.standings_cache[league_id] = get_standings(league_id)
        return self.standings_cache[league_id]

    def analyze_match(self, fixture):
        """Deep analysis of one match"""
        league_id = fixture['league_id']
        home_id = fixture['home_id']
        away_id = fixture['away_id']

        # Get data (3 calls per match)
        standings = self.get_league_standings(league_id)
        home_stats = get_team_stats(home_id, league_id)
        away_stats = get_team_stats(away_id, league_id)
        h2h = get_head_to_head(home_id, away_id)

        # Extract useful info
        home_form = home_stats.get('form', 'N/A')[-5:] if home_stats.get('form') else 'N/A'
        away_form = away_stats.get('form', 'N/A')[-5:] if away_stats.get('form') else 'N/A'

        # Simple but better scoring
        score = 50
        if "W" in home_form[-3:]:
            score += 14
        if "L" in away_form[-3:]:
            score += 12

        # Home advantage
        score += 9

        # H2H home dominance
        home_wins_h2h = sum(1 for m in h2h 
                           if m['teams']['home']['id'] == home_id and 
                           m['teams']['home']['winner'])
        if home_wins_h2h >= 3:
            score += 8

        confidence = min(93, max(58, score))

        return {
            "fixture": fixture,
            "home_form": home_form,
            "away_form": away_form,
            "h2h_games": len(h2h),
            "confidence": round(confidence, 1),
            "suggestion": self.generate_suggestion(confidence, fixture)
        }

    def generate_suggestion(self, confidence, fixture):
        if confidence >= 85:
            return "Strong home edge - consider Home Win or Home -1.5"
        elif confidence >= 78:
            return "Home Win or BTTS looks interesting"
        elif confidence >= 70:
            return "Watch for value on Under 2.5 or BTTS"
        else:
            return "Low confidence - proceed with caution"

    def run(self):
        print("🚀 Starting Pre-Match Analyzer v3 (Next 3 Days + Deep Analysis)\n")

        fixtures = get_fixtures_next_3_days()
        print(f"Total fixtures found: {len(fixtures)}")

        if not fixtures:
            send_telegram_message("No matches found in the next 3 days.")
            return

        analyzed = []
        for fx in fixtures:
            try:
                analysis = self.analyze_match(fx)
                analyzed.append(analysis)
                time.sleep(0.7)
            except Exception as e:
                if "API limit" in str(e):
                    break
                continue

        # Sort by confidence
        analyzed.sort(key=lambda x: x['confidence'], reverse=True)
        top_matches = analyzed[:20]

        today = datetime.now().strftime("%Y-%m-%d")

        # ===================== MESSAGE 1: Summary (15 matches) =====================
        msg1 = f"""<b>📊 GrokBet Pre-Match Analyzer</b>
📅 {today} | Next 3 Days

Top {min(15, len(top_matches))} Matches of Interest:

"""
        for i, m in enumerate(top_matches[:15], 1):
            fx = m['fixture']
            msg1 += f"{i}. <b>{fx['home_team']} vs {fx['away_team']}</b> ({fx['league']})\n"
            msg1 += f"   Confidence: <b>{m['confidence']}%</b>\n\n"

        msg1 += "See deep analysis in next message."

        send_telegram_message(msg1)

        # ===================== MESSAGE 2: Deep Dive (Top 6) =====================
        msg2 = f"<b>🔍 Deep Dive - Best 6 Matches</b>\n\n"

        for i, m in enumerate(top_matches[:6], 1):
            fx = m['fixture']
            msg2 += f"<b>{i}. {fx['home_team']} vs {fx['away_team']}</b>\n"
            msg2 += f"League: {fx['league']}\n"
            msg2 += f"Form: {m['home_form']} | {m['away_form']}\n"
            msg2 += f"H2H matches analyzed: {m['h2h_games']}\n"
            msg2 += f"Confidence: <b>{m['confidence']}%</b>\n"
            msg2 += f"Suggestion: {m['suggestion']}\n\n"

        msg2 += "Full data saved in GitHub."

        send_telegram_message(msg2)

        # Save to file
        with open("prematch_analysis.json", "w", encoding="utf-8") as f:
            json.dump({
                "date": today,
                "total_analyzed": len(top_matches),
                "matches": top_matches
            }, f, indent=2, ensure_ascii=False)

        print(f"\n✅ Done. Used approximately {api.calls} API calls.")


if __name__ == "__main__":
    analyzer = PreMatchAnalyzer()
    analyzer.run()